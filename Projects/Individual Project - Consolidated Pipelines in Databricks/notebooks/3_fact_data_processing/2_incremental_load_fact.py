...
base_path = f's3://sportsbar-sr/{data_source}'
...
# Check if landing directory has files before reading
try:
    files = dbutils.fs.ls(landing_path)
    csv_files = [f for f in files if f.name.endswith('.csv')]
    
    if not csv_files:
        print("No new CSV files found in landing directory. Skipping processing.")
        dbutils.notebook.exit("No new files to process")
except Exception as e:
    print(f"Exception type: {type(e).__name__}")
    print(f"Exception message: {e}")
    dbutils.notebook.exit("Landing directory not accessible")
...
# Cast order_qty to DOUBLE to match the existing table schema
df_casted = df.withColumn("order_qty", col("order_qty").cast("double"))
...
# Deduplicate df_joined before merge to avoid DELTA_MULTIPLE_SOURCE_ROW_MATCHING_TARGET_ROW_IN_MERGE
df_joined_dedup = df_joined.dropDuplicates(["order_placement_date", "order_id", "product_code", "customer_id"])
...
if not (spark.catalog.tableExists(silver_table)):
    ...
    silver_delta.alias("silver").merge(df_joined_dedup.alias("bronze"), "silver.order_placement_date = bronze.order_placement_date AND silver.order_id = bronze.order_id AND silver.product_code = bronze.product_code AND silver.customer_id = bronze.customer_id").whenMatchedUpdateAll().whenNotMatchedInsertAll().execute()
...
# Deduplicate df_gold before merge to avoid DELTA_MULTIPLE_SOURCE_ROW_MATCHING_TARGET_ROW_IN_MERGE
df_gold_dedup = df_gold.dropDuplicates(["date", "order_id", "product_code", "customer_code"])

if not (spark.catalog.tableExists(gold_table)):
    print("creating New Table")
    df_gold_dedup.write.format("delta").option(
        "delta.enableChangeDataFeed", "true"
    ).option("mergeSchema", "true").mode("overwrite").saveAsTable(gold_table)
else:
    gold_delta = DeltaTable.forName(spark, gold_table)
    gold_delta.alias("source").merge(df_gold_dedup.alias("gold"), "source.date = gold.date AND source.order_id = gold.order_id AND source.product_code = gold.product_code AND source.customer_code = gold.customer_code").whenMatchedUpdateAll().whenNotMatchedInsertAll().execute()
